<?php 
    include 'header.php';
?>
<div class="home-content">
        <div class="overview-boxes">
          <div class="box">
            <div class="right-side">
              <div class="box-topic">Commande</div>
              <div class="number"><?= getAllCommande()['nbre']?></div>
              <div class="indicator">
                <i class="bx bx-up-arrow-alt"></i>
                <span class="text">Depuis hier</span>
              </div>
            </div>
            <i class="bx bx-cart-alt cart"></i>
          </div>
          <div class="box">
            <div class="right-side">
              <div class="box-topic">Vente</div>
              <div class="number"><?= getAllVente()['nbre']?></div>
              <div class="indicator">
                <i class="bx bx-up-arrow-alt"></i>
                <span class="text">Depuis hier</span>
              </div>
            </div>
            <i class="bx bxs-cart-add cart two"></i>
          </div>
          <div class="box">
            <div class="right-side">
              <div class="box-topic">Article</div>
              <div class="number"><?= getAllArticle()['nbre']?></div>
              <div class="indicator">
                <i class="bx bx-up-arrow-alt"></i>
                <span class="text">Depuis hier</span>
              </div>
            </div>
            <i class="bx bx-cart cart three"></i>
          </div>
          <div class="box">
            <div class="right-side">
              <div class="box-topic">CA</div>
              <div class="number"><?= number_format(getAllCA()['prix'], '0',',',' ')?></div>
              <div class="indicator">
                <i class="bx bx-down-arrow-alt down"></i>
                <span class="text">Aujourd'hui</span>
              </div>
            </div>
            <i class="bx bxs-cart-download cart four"></i>
          </div>
        </div>

        <div class="sales-boxes">
          <div class="recent-sales box">
            <div class="title">Vente recentes</div>
            <?php         
                $ventes = getBestVente();    
            ?>

            <div class="sales-details">
              <ul class="details">
                <li class="topic">Date</li>
                <?php 
                  foreach ($ventes as $key => $value){
                          ?>
                          <li><a href="#"><?= date('d M Y', strtotime($value['date_vente']))?></a></li>
                          <?php                          
                  }           
                ?>
              </ul>
              <ul class="details">
                <li class="topic">Client</li>
                <li><a href="#"><?php 
                  foreach ($ventes as $key => $value){
                          ?>
                          <li><a href="#"><?= $value['nom']." ".$value['prenom']?></a></li>
                          <?php                          
                  }           
                ?></a></li> 
              </ul>
              <ul class="details">
                <li class="topic">Produit</li>
                <li><a href="#"><?php 
                  foreach ($ventes as $key => $value){
                          ?>
                          <li><a href="#"><?= $value['nom_article']?></a></li>
                          <?php                          
                  }           
                ?></a></li> 
              </ul>
              <ul class="details">
                <li class="topic">Prix</li>
                <li><a href="#"><?php 
                  foreach ($ventes as $key => $value){
                          ?>
                          <li style="text-align: end;"><a href="#"><?=number_format($value['prix'],0,","," " )?> CFA</a></li>
                          <?php                          
                  }           
                ?></a></li> 
              </ul>
            </div>
            <div class="button">
              <a href="#">Voir Tout</a>
            </div>
          </div>
          <div class="top-sales box">
            <div class="title">Produit les plus vendu</div>
            <ul class="top-sales-details">
            <?php         
                $vente = getMostVente();
                foreach ($vente as $key => $value){
                  ?>
                  <li>
                    <a href="#">
                      <!--<img src="images/sunglasses.jpg" alt="">-->
                      <span class="product"><?= $value['nom_article']?></span>
                    </a>
                    <span class="price"><?=number_format($value['prix'],0,","," " )?> CFA</span>
                  </li>
                  <?php  }    
            ?>              
            </ul>
          </div>
        </div>
      </div>
    </section>
<?php 
    include 'footer.php';
?>